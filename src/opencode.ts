...
export type OpenCodeOptions = { baseUrl?: string; fetch?: FetchLike; projectRoot: string; spawn?: typeof spawn; logger?: (message: string) => void };
export class OpenCodeAdapter {
  ...
  constructor(options: OpenCodeOptions) { this.baseUrl = (options.baseUrl ?? "http://127.0.0.1:4096").replace(/\/$/, ""); this.request = options.fetch ?? fetch; this.projectRoot = options.projectRoot; this.log = options.logger ?? (() => {}); this.launch = options.spawn ?? spawn; }
