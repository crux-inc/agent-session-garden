...
export type GardenServerOptions = { projectRoot: string; port?: number; clientFile?: string };
...
export class GardenServer {
  ...
  get port(): number {
    ...
    if (!address || typeof address === "string") throw new Error("Garden Server is not listening");
    return address.port;
  }
  ...
  async start(): Promise<void> {
    ...
    await listen(this.httpServer, this.requestedPort ?? 0);
    this.started = true;
  }
  ...
  private async route(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    if (request.method === "GET" && url.pathname === "/api/health") {
      this.json(response, 200, { ready: true, server: "ready", reconciliation: "disconnected", adapter: "disconnected" });
      return;
    }
    if (request.method === "GET" && url.pathname === "/api/sessions") {
      this.json(response, 200, { schemaVersion: 1, project: { root: this.projectRoot }, sessions: [] });
      return;
    }
